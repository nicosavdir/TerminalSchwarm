...
class Homie :
  ...
  def think(self):
      viewed=self.view()
      if viewed.__contains__(8) or viewed.__contains__(12):
          self.do=10
...
def buildWall(ascii):
    for i in range(2):
        for j in range(size[0]):
            changeGrid(j,0,ascii)
            changeGrid(j,size[1]-1,ascii)
        for j in range(size[1]):
            changeGrid(0,j,ascii)
            changeGrid(size[0]-1,j,ascii)
